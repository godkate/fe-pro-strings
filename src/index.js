...
export const changeWord = (string, word, newWord) => {
  ...
  if (string.includes(word)) {
    result = `${string.slice(0, string.indexOf(word))}${newWord}${string.slice(string.indexOf(word) + word.length)}`;
  } else {
    result = string;
  }
...
export const quantityOfSymbolsWithIndexOf = (string, symbol) => {
  let result = 0;
  let position = string.toLowerCase().indexOf(symbol.toLowerCase());
  while (true) {
    if (position === -1) {
      break;
    }
    result += 1;
    position = string.toLowerCase().indexOf(symbol.toLowerCase(), position +1);
  }
  return result;
};
